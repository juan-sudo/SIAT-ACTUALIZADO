...
class ModeloPisos
{
   ...
   public static function mdlNuevoPisoCons($tabla, $datos)
   {
      ...
      //------Obtener Id_Predio
      $stmt = $pdo1->prepare("SELECT Id_Predio FROM predio WHERE Id_Catastro=:Id_Catastro AND Id_Anio=:Id_Anio");
      ...
      //--- Insertar Piso
      try {
         $stmt1 = $pdo1->prepare("INSERT INTO $tabla(Catastro_Piso, Numero_Piso, Incremento, Fecha_Construccion, Cantidad_Anios, Valores_Unitarios, Porcentaje_Depreciacion,	Valor_Unitario_Depreciado,	Area_Construida, Valor_Area_Construida, Areas_Comunes, Valor_Areas_Comunes, Valor_Construida, Categorias_Edificacion, Id_Estado_Conservacion, Id_Clasificacion_Piso, Id_Material_Piso, Id_Predio,Id_Construccion) VALUES (:Catastro_Piso, :Numero_Piso, :Incremento, :Fecha_Construccion, :Cantidad_Anios, :Valores_Unitarios, :Porcentaje_Depreciacion, :Valor_Unitario_Depreciado, :Area_Construida, :Valor_Area_Construida, :Areas_Comunes, :Valor_Areas_Comunes, :Valor_Construida, :Categorias_Edificacion, :Id_Estado_Conservacion, :Id_Clasificacion_Piso, :Id_Material_Piso, :Id_Predio, :Id_Construccion)");
         $stmt1->bindParam(":Catastro_Piso", $datos['Catastro_Piso']);
         $stmt1->bindParam(":Numero_Piso", $datos['numPiso']);
         ...
         $stmt1->bindParam(":Id_Construccion", $datos['idConstruccion']);
 ...
 public static function mdlMostrarConstruccion($datos)
{
   
   // $stmt0 = Conexion::conectar()->prepare("SELECT * FROM construccion WHERE Id_Predio = :idPredio");
   
      $stmt0 = Conexion::conectar()->prepare("SELECT c.Id_Construccion as Id_Cons, c.Id_Predio as idPre,c.Nombre_Construccion, c.numero_construccion ,s.*
            FROM  construccion c 
            LEFT JOIN pisos s ON  c.Id_Construccion = s.Id_Construccion
            WHERE c.Id_Predio = :idPredio
            ORDER BY s.Numero_Piso, c.numero_construccion");

    $stmt0->bindParam(":idPredio", $datos['idPredio']);
    $stmt0->execute();
      ...
      if ($datos != null) {
         ...
         try {
           ...
           // $stmt0 = $pdo->prepare("SELECT * FROM pisos WHERE Id_Predio=:Id_Predio order by Numero_Piso");
            ...
            //  $stmt0 = $pdo->prepare("SELECT s.*, c.*
            //    FROM pisos s
            //    LEFT JOIN construccion c ON s.Id_Construccion = c.Id_Construccion
            //    WHERE s.Id_Predio = :Id_Predio
            //    ORDER BY s.Numero_Piso, c.numero_construccion");


            $stmt0 = $pdo->prepare("SELECT s.*,c.Id_Predio as id_predio_c, c.Nombre_Construccion,c.Observaciones,c.numero_construccion
            FROM pisos s
            LEFT JOIN construccion c ON  s.Id_Construccion = c.Id_Construccion
            WHERE s.Id_Predio = :Id_Predio AND c.Id_Construccion IS NULL
            ORDER BY s.Numero_Piso, c.numero_construccion");
