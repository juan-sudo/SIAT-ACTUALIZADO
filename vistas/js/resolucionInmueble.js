  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_inmueble option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaInmueble tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaInmueble tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaInmueble tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaInmueble tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaInmueble tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaInmueble tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaInmueble tbody tr");
  ...
  if (
    ...
    //  console.log("Se encontró al menos un valor válido. Se muestra el modal de impresión.");
     // $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           $("#modal_coactiva_dos_inmueble").modal("show");
      ...
      $(".print_orden_aviso_ok_16").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       resolucionInmueble_.captureModalData();
  ...
  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("dniPropietario", resolucionInmueble_.dniPropietario);
        ...
        .finally(() => {
          ...
          $('#modal_coactiva_dos_inmueble').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           resolucionInmueble_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_coactiva_dos_inmueble').on('hidden.bs.modal', function () {
          resolucionInmueble_.limpiarCampos();
      });
...
const resolucionInmueble_ = new ResolucionInmueble();
...
$("#anio_orden_inmueble").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaInmueble tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resolucionInmueble_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaInmueble thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resolucionInmueble_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirInmueble", function () {
  ...
  resolucionInmueble_.imprimirherecoactivo();
});



//CERRAR VENTANA ES TA VACIO DE COACTIVO CON OK.
$(document).on("click", ".anio_orden_inmueble", function() {
  // Cerrar el modal
  $('#modal_vacio_coactivo').modal('hide');
});

//CLIK PARA MODAL DE ESTADO DE CUENTA COACTIVO
$("#abrirResolucionDOSIm").click(function () {
  $("#modalResolucionInmueble").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_inmueble", function () {
  //console.log("estas aqui");
  resolucionInmueble_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalResolucionInmueble').on('show.bs.modal', function () {
   resolucionInmueble_.muestra_deuda();
 });
...
setInterval(resolucionInmueble_.eliminarArchivosPDF, 60000);
