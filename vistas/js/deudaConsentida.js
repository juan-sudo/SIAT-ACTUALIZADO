  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_consentida_deuda option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaConcentidaDeuda tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaConcentidaDeuda tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaConcentidaDeuda tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaConcentidaDeuda tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaConcenDeua tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
      ...
      $("#modal_concentida_deuda").modal("show");
      ...
      $(".print_orden_aviso_ok_12").click(function() {

        console.log("Botón OK clickeado -----");
          // Capturamos los datos del formulario
        
       consentida_deuda_.captureModalData();
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("detalleDocumentos", JSON.stringify(consentida_deuda_.detalleDocumentos));
        ...
        // Usar fetch en lugar de $.ajax
          fetch("./vistas/print/imprimirDeudaConcentida.php", {
              method: "POST",
              body: datos
          })
          .then(response => {
          ...
          $('#modal_concentida_deuda').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           consentida_deuda_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_concentida_deuda').on('hidden.bs.modal', function () {
          consentida_deuda_.limpiarCampos();
      });
...
//////////////////////FIN MODAL COACTIVO///////////////////////////////
 ...
 captureModalData() {
    ...
    $('#consentida-deuda tbody tr').each((index, row) => {  // Usamos función flecha para mantener el contexto de `this`
        // Obtener los valores de los inputs
        let detalle = $(row).find('input[name="detalleFraccionamiento12"]').val();
        ...
        if (detalle ) {
            ...
            this.detalleDocumentos.push(filaData);
    ...
    console.log('Detalle Perdida:', this.detalleDocumentos);
}
...
const consentida_deuda_ = new ConcentidaDeuda();
...
$("#anio_orden_consentida_deuda").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaConcenDeua tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  consentida_deuda_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaConcenDeua thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  consentida_deuda_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirConcentidaDeu", function () {
  ...
  consentida_deuda_.imprimirherecoactivo();
});
...
$("#abrirConcentidaDeuda").click(function () {
  $("#modalConcentidaDeuda").modal("show");
});

//AGREGAR TR CON AGREGAR FILA
$(document).ready(function() {
    // Agregar fila
    $('.agregar-fila-concentido').click(function() {
        var nuevaFila = `
        <tr>
            <td><input type="text" class="form-control" name="detalleFraccionamiento12" id="detalleFraccionamiento12" placeholder="Detalle de fraccionamiento"></td>

            <td><button type="button" class="eliminar-fila_d">X</button></td>
        </tr>
        `;
        $('#consentida-deuda tbody').append(nuevaFila);
    });

    // Eliminar fila
    $(document).on('click', '.eliminar-fila_d', function() {
...
$(document).on("change", "#anio_orden_consentida_deuda", function () {
  //console.log("estas aqui");
  consentida_deuda_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalConcentidaDeuda').on('show.bs.modal', function () {
   consentida_deuda_.muestra_deuda();
 });
...
setInterval(consentida_deuda_.eliminarArchivosPDF, 60000);
