class ResolucionMedidaCautelar {
  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_acumulacion option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaAcumulacionn tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaAcumulacionn tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaAcumulacionn tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaAcumulacionn tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaAcumulacionn tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaAcumulacionn tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  ...
  console.log("Diste click en el encabezado de la tabla");


  const filas = $("#primeraTablaAcumulacione tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
    ...
    //  console.log("Se encontró al menos un valor válido. Se muestra el modal de impresión.");
     // $("#Modalimprimir_cuenta_coactivo").modal("show");
      ...
      $("#modal_coactiva_dos_cautelar").modal("show");
      ...
      $(".print_orden_aviso_ok_17").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       resolucionMedidaCautelar_.captureModalData();
  ...
  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("diasHabiles", resolucionMedidaCautelar_.diasHabiles);
        ...
        fetch("./vistas/print/imprimirResolucionMedidaCautelar.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_coactiva_dos_cautelar').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           resolucionMedidaCautelar_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_coactiva_dos_cautelar').on('hidden.bs.modal', function () {
          resolucionMedidaCautelar_.limpiarCampos();
      });
  ...
  // // Usar fetch en lugar de $.ajax
  // fetch("./vistas/print/imprimirResolucionInmueble.php", {
  //     method: "POST",
  //     body: datos
  // })
  // .then(response => {
...
const resolucionMedidaCautelar_ = new ResolucionMedidaCautelar();
...
$("#anio_orden_acumulacion").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaAcumulacione tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resolucionMedidaCautelar_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaAcumulacione thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resolucionMedidaCautelar_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirAcumulaciona", function () {
  ...
  resolucionMedidaCautelar_.imprimirherecoactivo();
});



//CERRAR VENTANA ES TA VACIO DE COACTIVO CON OK.
$(document).on("click", ".anio_orden_acumulacion", function() {
  // Cerrar el modal
  $('#modal_vacio_coactivo').modal('hide');
});

//CLIK PARA MODAL DE ESTADO DE CUENTA COACTIVO
$("#abrirResolucionDOSMedidaCau").click(function () {
  $("#modalrResolucionAcumulacionEjecucion").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_acumulacion", function () {
  //console.log("estas aqui");
  resolucionMedidaCautelar_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalrResolucionAcumulacionEjecucion').on('show.bs.modal', function () {
   resolucionMedidaCautelar_.muestra_deuda();
 });
...
setInterval(resolucionMedidaCautelar_.eliminarArchivosPDF, 60000);
