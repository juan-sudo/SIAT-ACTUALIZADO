  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_coactivo_reco option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablacRecor tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablacRecor tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablacRecor tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablacRecor tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablacRecor tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablacRecor tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablacRecor tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
    ...
    $("#modal_formulario_carta").modal("show");
      ...
      $(".print_orden_coactivo_aviso_ok").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       cartaRecodatotio_.captureModalData();

          // Validar todos los campos
        if (!cartaRecodatotio_.tipoTributario || cartaRecodatotio_.tipoTributario === "") {
            alert("Por favor, seleccione el tipo tributario.");
            return; // Detener ejecución si el campo tipoTributario está vacío
        }

      
     if (!cartaRecodatotio_.numeroCuota || cartaRecodatotio_.numeroCuota === "" || cartaRecodatotio_.numeroCuota <= 0) {
        ...
        if (!cartaRecodatotio_.anioFiscalg || cartaRecodatotio_.anioFiscalg === "") {
            alert("Por favor, seleccione un año fiscal.");
            return; // Detener ejecución si el campo anioFiscalg está vacío
        }

        if (!cartaRecodatotio_.fechaVencimeinto || cartaRecodatotio_.fechaVencimeinto === "") {
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      datos.append("tipoTributario", cartaRecodatotio_.tipoTributario);
      datos.append("numeroCuota", cartaRecodatotio_.numeroCuota);
      datos.append("anioFiscalg", cartaRecodatotio_.anioFiscalg);
      datos.append("fechaVencimiento", cartaRecodatotio_.fechaVencimeinto);
        ...
        fetch("./vistas/print/imprimirCartaRecordatorio.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_formulario_carta').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           cartaRecodatotio_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_formulario_carta').on('hidden.bs.modal', function () {
          cartaRecodatotio_.limpiarCampos();
      });
...
//CAPTURAR LOS VALORES DE MODAL
  captureModalData() {
    this.tipoTributario = document.getElementById("tipoTributario").value;
    this.numeroCuota = document.getElementById("numeroCuota").value;
    this.fechaVencimeinto = document.getElementById("fechaVencimiento").value;
     this.anioFiscalg = document.getElementById("anioFiscalg").value;
...
const cartaRecodatotio_ = new CartaRecordatorio();
...
$("#anio_orden_coactivo_reco").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablacRecor tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  cartaRecodatotio_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablacRecor thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  cartaRecodatotio_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirCartaRecordatorio", function () {
  ...
  cartaRecodatotio_.imprimirherecoactivo();
});
...
$("#abrirCartaRecordatorio").click(function () {
  $("#modalCartaRecordatorio").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_coactivo_reco", function () {
  //console.log("estas aqui");
  cartaRecodatotio_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalCartaRecordatorio').on('show.bs.modal', function () {
   cartaRecodatotio_.muestra_deuda();
 });
...
setInterval(cartaRecodatotio_.eliminarArchivosPDF, 60000);
