...
class ResolucionFraccionamiento {
  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_coactivo_per option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablacReso tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablacReso tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablacReso tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablacReso tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablacReso tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablacReso tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablacRoso tbody tr");
...
imprimirherecoactivo() {
  ...
  fetch("./vistas/print/imprimirResoFraccionamiento.php", {
      method: "POST",
      body: datos
  })
  .then(response => {
...
const resoFraccionamiento_ = new ResolucionFraccionamiento();
...
$("#anio_orden_coactivo_reso").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablacRoso tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resoFraccionamiento_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablacRoso thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resoFraccionamiento_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirResoFraccionamiento", function () {
  ...
  resoFraccionamiento_.imprimirherecoactivo();
});
...
$("#abrirAproFraccio").click(function () {
  $("#modalrResolucionFraccionado").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_coactivo_reso", function () {
  //console.log("estas aqui");
  resoFraccionamiento_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalrResolucionFraccionado').on('show.bs.modal', function () {
   resoFraccionamiento_.muestra_deuda();
 });
...
setInterval(resoFraccionamiento_.eliminarArchivosPDF, 60000);
