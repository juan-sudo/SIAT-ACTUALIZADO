  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_vehicular option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaVehicular tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaVehicular tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaVehicular tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaVehicular tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaVehicular tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaVehicular tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaVehicular tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
     ...
     // $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           $("#modal_coactiva_dos_vehicular").modal("show");
      ...
      $(".print_orden_aviso_ok_15").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       resolucionVehicular_.captureModalData();
  ...
  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("dniPropietario", resolucionVehicular_.dniPropietario);
        ...
        fetch("./vistas/print/imprimirResolucionVehicular.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_coactiva_dos_vehicular').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           resolucionVehicular_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_coactiva_dos_vehicular').on('hidden.bs.modal', function () {
          resolucionVehicular_.limpiarCampos();
      });
...
const resolucionVehicular_ = new ResolucionVehicular();
...
$("#anio_orden_vehicular").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaVehicular tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resolucionVehicular_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaVehicular thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resolucionVehicular_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirVehicular", function () {
  ...
  resolucionVehicular_.imprimirherecoactivo();
});



//CERRAR VENTANA ES TA VACIO DE COACTIVO CON OK.
$(document).on("click", ".anio_orden_vehicular", function() {
  // Cerrar el modal
  $('#modal_vacio_coactivo').modal('hide');
});

//CLIK PARA MODAL DE ESTADO DE CUENTA COACTIVO
$("#abrirResolucionDOSVehi").click(function () {
  $("#modalResolucionVehicular").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_vehicular", function () {
  //console.log("estas aqui");
  resolucionVehicular_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalResolucionVehicular').on('show.bs.modal', function () {
   resolucionVehicular_.muestra_deuda();
 });
...
setInterval(resolucionVehicular_.eliminarArchivosPDF, 60000);
