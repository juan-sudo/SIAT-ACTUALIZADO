  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_coactivopagono option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaPagoReque tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaPagoReque tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaPagoReque tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaPagoReque tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaPagoReque tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaPagoReque tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaRequePaga tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
      ...
      // $("#Modalimprimir_cuenta_coactivo").modal("show");
       ...
       $("#modal_requerimiento_pago").modal("show");
      ...
      $(".print_orden_aviso_ok_2").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       pagoRequerimiento_.captureModalData();
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("numeroCuota", pagoRequerimiento_.numeroCuota);
      datos.append("anioFiscalg", pagoRequerimiento_.anioFiscalg);
      datos.append("fechaVencimiento", pagoRequerimiento_.fechaVencimeinto);
      datos.append("periodo", pagoRequerimiento_.periodo);
      datos.append("plazodias", pagoRequerimiento_.plazodias);
        ...
        fetch("./vistas/print/imprimirRequerimientoPago.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_requerimiento_pago').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           pagoRequerimiento_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_requerimiento_pago').on('hidden.bs.modal', function () {
          pagoRequerimiento_.limpiarCampos();
      });
...
//////////////////////FIN MODAL COACTIVO///////////////////////////////
 ...
 captureModalData() {
    ...
    this.numeroCuota = document.getElementById("numeroCuota2").value;
    this.fechaVencimeinto = document.getElementById("fechaVencimiento2").value;
    this.anioFiscalg = document.getElementById("anioFiscalg2").value;
    this.periodo = document.getElementById("periodo2").value;
    this.plazodias = document.getElementById("plazoDias2").value;
...
const pagoRequerimiento_ = new PagoRequerimiento();
...
$("#anio_orden_coactivopagono").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaRequePaga tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  pagoRequerimiento_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaRequePaga thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  pagoRequerimiento_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirRequerimientoPago", function () {
  ...
  pagoRequerimiento_.imprimirherecoactivo();
});
...
$("#abrirRequerimientoPago").click(function () {
  console.log("aqui llego------------");
  $("#modalRequerimientoPago").modal("show");
});


//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_coactivopagono", function () {
  //console.log("estas aqui");
  pagoRequerimiento_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalRequerimientoPago').on('show.bs.modal', function () {
   pagoRequerimiento_.muestra_deuda();
 });
...
setInterval(pagoRequerimiento_.eliminarArchivosPDF, 60000);
