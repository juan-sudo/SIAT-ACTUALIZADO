  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_valores option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaValores tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaValores tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaValores tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaValores tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaValores tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaValores tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaValores tbody tr");
...
imprimirherecoactivo() {
  ...
  fetch("./vistas/print/imprimirNotificacionValores.php", {
      method: "POST",
      body: datos
  })
  .then(response => {
...
const notificacionValores_ = new NotificacionValores();
...
$("#anio_orden_valores").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaValores tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  notificacionValores_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaValores thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  notificacionValores_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirValores", function () {
  ...
  notificacionValores_.imprimirherecoactivo();
});



//CERRAR VENTANA ES TA VACIO DE COACTIVO CON OK.
$(document).on("click", ".anio_orden_valores", function() {
  // Cerrar el modal
  $('#modal_vacio_coactivo').modal('hide');
});

//CLIK PARA MODAL DE ESTADO DE CUENTA COACTIVO
$("#abrirResoNotiValores").click(function () {
  $("#modalNotificacionValores").modal("show");
});

//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_valores", function () {
  //console.log("estas aqui");
  notificacionValores_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalNotificacionValores').on('show.bs.modal', function () {
   notificacionValores_.muestra_deuda();
 });
...
setInterval(notificacionValores_.eliminarArchivosPDF, 60000);
