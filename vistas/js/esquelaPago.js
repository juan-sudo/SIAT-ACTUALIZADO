  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_esquela option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaEsquelaPago tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaEsquelaPago tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaEsquelaPago tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaEsquelaPago tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaEsquelaPago tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaEsquelaPago tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaEsquelPago tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
      ...
      // $("#Modalimprimir_cuenta_coactivo").modal("show");
       ...
       $("#modal_esquela_cobranza").modal("show");
      ...
      $(".print_orden_aviso_ok_3").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       esquelaPago_.captureModalData();
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("plazodias", esquelaPago_.plazodias);
        ...
        fetch("./vistas/print/imprimirEsquelaPago.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_esquela_cobranza').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           esquelaPago_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_esquela_cobranza').on('hidden.bs.modal', function () {
          esquelaPago_.limpiarCampos();
      });
...
//////////////////////FIN MODAL COACTIVO///////////////////////////////
 ...
 captureModalData() {
    ...
    this.plazodias = document.getElementById("plazoDias3").value;
...
const esquelaPago_ = new EsquelaPago();
...
$("#anio_orden_esquela").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaEsquelPago tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  esquelaPago_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaEsquelPago thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  esquelaPago_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirEsquelaPago", function () {
  ...
  esquelaPago_.imprimirherecoactivo();
});
...
$("#abrirEsquelaCobranza").click(function () {
  console.log("aqui llego------------");
  $("#modalEsquelaCobranza").modal("show");
});

//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_esquela", function () {
  //console.log("estas aqui");
  esquelaPago_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalEsquelaCobranza').on('show.bs.modal', function () {
   esquelaPago_.muestra_deuda();
 });
...
setInterval(esquelaPago_.eliminarArchivosPDF, 60000);
