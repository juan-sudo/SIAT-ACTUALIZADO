...
actualizarTotales() {
 ...
 $("#segundaTablacAcuVehi tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablacAcuVehi tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablacAcuVehi tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablacAcuVehi tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablacAcuVehi tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablacAcuVehi tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablacAcumVehi tbody tr");
...
const resoAcumulacionVehi_ = new ResolucionAculacionVehi();
...
$("#anio_orden_coactivo_acuve").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablacAcumVehi tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resoAcumulacionVehi_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablacAcumVehi thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resoAcumulacionVehi_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirAcuVehiFraccionamiento", function () {
  ...
  resoAcumulacionVehi_.imprimirherecoactivo();
});
...
$("#abrirResolucionDOSVehi").click(function () {
  $("#modalrResolucionAcumulacionVehi").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_coactivo_acuve", function () {
  //console.log("estas aqui");
  resoAcumulacionVehi_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalrResolucionAcumulacionVehi').on('show.bs.modal', function () {
   resoAcumulacionVehi_.muestra_deuda();
 });
...
setInterval(resoAcumulacionVehi_.eliminarArchivosPDF, 60000);
