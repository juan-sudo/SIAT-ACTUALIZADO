  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_coactivo_eduda option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablacResoDeuda tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablacResoDeuda tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablacResoDeuda tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablacResoDeuda tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablacResoDeuda tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablacResoDeuda tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablacRosoDeuda tbody tr");
...
imprimirherecoactivo() {
  ...
  fetch("./vistas/print/imprimirDeudaFraccionamiento.php", {
      method: "POST",
      body: datos
  })
  .then(response => {
...
const deudaFraccionamiento_ = new DeudaFraccionamiento();
...
$("#anio_orden_coactivo_eduda").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablacRosoDeuda tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  deudaFraccionamiento_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablacRosoDeuda thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  deudaFraccionamiento_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirResoDeuda", function () {
  ...
  deudaFraccionamiento_.imprimirherecoactivo();
});
...
$("#abrirFraccionadoDeuda").click(function () {
  $("#modalFraccionadoDeuda").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_coactivo_eduda", function () {
  //console.log("estas aqui");
  deudaFraccionamiento_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalFraccionadoDeuda').on('show.bs.modal', function () {
   deudaFraccionamiento_.muestra_deuda();
 });
...
setInterval(deudaFraccionamiento_.eliminarArchivosPDF, 60000);
