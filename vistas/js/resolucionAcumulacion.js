  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_reso_acu option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaResoDosAcu tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaResoDosAcu tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaResoDosAcu tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaResoDosAcu tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaResoDosAcu tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaResoDosAcu tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaResoDosAcu tbody tr");
  ...
  if (
    ...
    //  console.log("Se encontró al menos un valor válido. Se muestra el modal de impresión.");
     // $("#Modalimprimir_cuenta_coactivo").modal("show");
      ...
      $(".print_orden_aviso_ok_14").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       resoAcumulacion_.captureModalData();
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("expediente", resoAcumulacion_.expedientes);
      datos.append("numeroExpediente", resoAcumulacion_.numeroExpediente);
        ...
        .finally(() => {
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           resoAcumulacion_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_coactiva_dos').on('hidden.bs.modal', function () {
          resoAcumulacion_.limpiarCampos();
      });
...
captureModalData() {
    ...
    this.expedientes = document.getElementById("expedientes14").value;
    this.numeroExpediente = document.getElementById("numeroExpediente14").value;
...
const resoAcumulacion_ = new ResolucionAculacion();
...
$("#anio_orden_reso_acu").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaResoDosAcu tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resoAcumulacion_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaResoDosAcu thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resoAcumulacion_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirResoDosAcu", function () {
  ...
  resoAcumulacion_.imprimirherecoactivo();
});
...
$("#abrirResolucionDOS").click(function () {
  $("#modalrResolucionAcumulacionDOS").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_reso_acu", function () {
  //console.log("estas aqui");
  resoAcumulacion_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalrResolucionAcumulacionDOS').on('show.bs.modal', function () {
   resoAcumulacion_.muestra_deuda();
 });
...
setInterval(resoAcumulacion_.eliminarArchivosPDF, 60000);
