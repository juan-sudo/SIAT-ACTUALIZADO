...
class ResolucionMulta {
  ...
  constructor() {
    ...
    this.docuIfraccion = null;
    this.fechaInfraccion = null;
    this.baseImponible = null;
    this.sancion = null;
    this.montoInsoluto = null;
    this.interesMoratorio = null;
    this.total = null;
  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_reso_multa option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaResoMulta tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaResoMulta tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaResoMulta tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaResoMulta tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaResoMulta tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaResoMulta tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaResoMulta tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
     ...
     // $("#Modalimprimir_cuenta_coactivo").modal("show");
       ...
       $("#modal_resolucion_multa").modal("show");
      ...
      $(".print_orden_aviso_ok_8").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       resolucion_multa_.captureModalData();

          // Validar todos los campos
    

    //  if (!resolucion_multa_.numeroCuota || resolucion_multa_.numeroCuota === "" || resolucion_multa_.numeroCuota <= 0) {
    ...
    // }
    //     if (!resolucion_multa_.anioFiscalg || resolucion_multa_.anioFiscalg === "") {
    //         alert("Por favor, seleccione un año fiscal.");
    //         return; // Detener ejecución si el campo anioFiscalg está vacío
    //     }

    //     if (!resolucion_multa_.fechaVencimeinto || resolucion_multa_.fechaVencimeinto === "") {
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("docuIfraccion", resolucion_multa_.docuIfraccion);
      datos.append("fechaInfraccion", resolucion_multa_.fechaInfraccion);
      datos.append("baseImponible", resolucion_multa_.baseImponible);
      datos.append("sancion", resolucion_multa_.sancion);
      datos.append("montoInsoluto", resolucion_multa_.montoInsoluto);
      datos.append("interesMoratorio", resolucion_multa_.interesMoratorio);
      datos.append("total", resolucion_multa_.total);
        ...
        fetch("./vistas/print/imprimirResolucionMulta.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_resolucion_multa').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           resolucion_multa_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_resolucion_multa').on('hidden.bs.modal', function () {
          resolucion_multa_.limpiarCampos();
      });
  ...
  // // Usar fetch en lugar de $.ajax
  // fetch("./vistas/print/imprimirResolucionDeterminacion.php", {
  //     method: "POST",
  //     body: datos
  // })
  // .then(response => {
...
//////////////////////FIN MODAL COACTIVO///////////////////////////////
 ...
 captureModalData() {
    ...
    this.docuIfraccion = document.getElementById("docInfraccion8").value;
    this.fechaInfraccion = document.getElementById("fechaInfraccion8").value;
    this.baseImponible = document.getElementById("baseImponible8").value;
    this.sancion = document.getElementById("sansion8").value;
    this.montoInsoluto = document.getElementById("montoInsoluto8").value;
    this.interesMoratorio = document.getElementById("interesMoratorio8").value;
    this.total = document.getElementById("total8").value;
...
const resolucion_multa_ = new ResolucionMulta();
...
$("#anio_orden_reso_multa").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaResoMulta tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resolucion_multa_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaResoMulta thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resolucion_multa_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirResoMulta", function () {
  ...
  resolucion_multa_.imprimirherecoactivo();
});
...
$("#abrirResoMulta").click(function () {
  console.log("aqui llego------------");
  $("#modalResolucionMulta").modal("show");
});

//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_reso_multa", function () {
  //console.log("estas aqui");
  resolucion_multa_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalResolucionMulta').on('show.bs.modal', function () {
   resolucion_multa_.muestra_deuda();
 });
...
setInterval(resolucion_multa_.eliminarArchivosPDF, 60000);
