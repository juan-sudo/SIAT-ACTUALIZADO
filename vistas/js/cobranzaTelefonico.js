  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_gestion_cotelefono option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaCoTelefonica tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaCoTelefonica tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaCoTelefonica tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaCoTelefonica tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaCoTelefonica tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaCoTelefonica tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaCoTelefo tbody tr");
...
imprimirherecoactivo() {
  ...
  fetch("./vistas/print/imprimirCobranzaTelefonica.php", {
      method: "POST",
      body: datos
  })
  .then(response => {
...
const cobranza_telefonica_ = new CobranzaTelefonica();
...
$("#anio_orden_gestion_cotelefono").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaCoTelefo tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  cobranza_telefonica_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaCoTelefo thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  cobranza_telefonica_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirCobranzaTelefonica", function () {
  ...
  cobranza_telefonica_.imprimirherecoactivo();
});
...
$("#abrirCobranzaTelefonica").click(function () {
  console.log("aqui llego------------");
  $("#modalCobranzaTelefonica").modal("show");
});

//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_gestion_cotelefono", function () {
  //console.log("estas aqui");
  cobranza_telefonica_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalCobranzaTelefonica').on('show.bs.modal', function () {
   cobranza_telefonica_.muestra_deuda();
 });
...
setInterval(cobranza_telefonica_.eliminarArchivosPDF, 60000);
