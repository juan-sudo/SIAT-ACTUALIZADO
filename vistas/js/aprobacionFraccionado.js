  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_aprobacion option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablacResoApro tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablacResoApro tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablacResoApro tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablacResoApro tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablacResoApro tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablacResoApro tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablacRosoApro tbody tr");
  ...
  if (
     ...
     // $("#Modalimprimir_cuenta_coactivo").modal("show");
      ...
      // $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           $("#modal_aprobacion_fracciona").modal("show");
      ...
      $(".print_orden_aviso_ok_11").click(function() {

        console.log("Botón OK clickeado -----");
          // Capturamos los datos del formulario
        
       aproFraccionamiento_.captureModalData();
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("importeFraccionado", aproFraccionamiento_.importeFraccionado);
        ...
        fetch("./vistas/print/imprimirAproFraccionamiento.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_aprobacion_fracciona').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           aproFraccionamiento_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_aprobacion_fracciona').on('hidden.bs.modal', function () {
          aproFraccionamiento_.limpiarCampos();
      });
...
const aproFraccionamiento_ = new AproFraccionamiento();
...
$("#anio_orden_aprobacion").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablacRosoApro tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  aproFraccionamiento_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablacRosoApro thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  aproFraccionamiento_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirResoApro", function () {
  ...
  aproFraccionamiento_.imprimirherecoactivo();
});
...
$("#abrirAproFraccion").click(function () {
  $("#modalAprobacionFracciona").modal("show");
});



//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_aprobacion", function () {
  //console.log("estas aqui");
  aproFraccionamiento_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalAprobacionFracciona').on('show.bs.modal', function () {
   aproFraccionamiento_.muestra_deuda();
 });
...
setInterval(aproFraccionamiento_.eliminarArchivosPDF, 60000);
