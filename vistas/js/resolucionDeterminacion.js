...
class ResolucionDetermi {
  ...
  constructor() {
    ...
    this.montoFiscalizado = null;
    this.impuestoFiscalizado = null;
  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_reso_determinacion option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaResoDete tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaResoDete tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaResoDete tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaResoDete tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaResoDete tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaResoDete tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaResoDete tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
     ...
     // $("#Modalimprimir_cuenta_coactivo").modal("show");
       ...
       $("#modal_resolucion_determinacion").modal("show");
      ...
      $(".print_orden_aviso_ok_7").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       resolucion_determinacion_.captureModalData();

          // Validar todos los campos
    

    //  if (!resolucion_determinacion_.numeroCuota || resolucion_determinacion_.numeroCuota === "" || resolucion_determinacion_.numeroCuota <= 0) {
    ...
    // }
    //     if (!resolucion_determinacion_.anioFiscalg || resolucion_determinacion_.anioFiscalg === "") {
    //         alert("Por favor, seleccione un año fiscal.");
    //         return; // Detener ejecución si el campo anioFiscalg está vacío
    //     }

    //     if (!resolucion_determinacion_.fechaVencimeinto || resolucion_determinacion_.fechaVencimeinto === "") {
    ...
    //  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("baseImponible", resolucion_determinacion_.baseImponible);
      datos.append("montoFiscalizado", resolucion_determinacion_.montoFiscalizado);
      datos.append("impuestoFiscalizado", resolucion_determinacion_.impuestoFiscalizado);
        ...
        .finally(() => {
          ...
          $('#modal_resolucion_determinacion').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           resolucion_determinacion_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_resolucion_determinacion').on('hidden.bs.modal', function () {
          resolucion_determinacion_.limpiarCampos();
      });
...
//////////////////////FIN MODAL COACTIVO///////////////////////////////
 ...
 captureModalData() {
    ...
    this.baseImponible = document.getElementById("baseImponible7").value;
    this.montoFiscalizado = document.getElementById("montoFiscalizado7").value;
    this.impuestoFiscalizado = document.getElementById("impuestoFiscalizado7").value;
...
const resolucion_determinacion_ = new ResolucionDetermi();
...
$("#anio_orden_reso_determinacion").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaResoDete tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  resolucion_determinacion_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaResoDete thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  resolucion_determinacion_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirResoDeter", function () {
  ...
  resolucion_determinacion_.imprimirherecoactivo();
});
...
$("#abrirResoDeter").click(function () {
  console.log("aqui llego------------");
  $("#modalResolucionDeterminacion").modal("show");
});

//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_reso_determinacion", function () {
  //console.log("estas aqui");
  resolucion_determinacion_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalResolucionDeterminacion').on('show.bs.modal', function () {
   resolucion_determinacion_.muestra_deuda();
 });
...
setInterval(resolucion_determinacion_.eliminarArchivosPDF, 60000);
