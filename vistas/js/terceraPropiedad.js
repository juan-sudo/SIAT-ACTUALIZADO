  ...
  constructor() {
    ...
    this.totalTotalI = 0;
     ...
     this.resolucionMedida=null;
      ...
      this.ubicacionPredio=null;
      this.partidaRegistral=null;
      this.fundamento=null;


     this.anexos=[];
     this.numebleInmueble=[];
  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_propiedad option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaPropiedad tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaPropiedad tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaPropiedad tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaPropiedad tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaPropiedad tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaPropiedad tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaPropiedad tbody tr");
...
imprimirherecoactivo() {
  ...
  if (
      ...
      $("#modal_terceria_propiedad").modal("show");
      ...
      $(".print_orden_aviso_ok_19").click(function() {

        console.log("Botón OK clickeado");
          // Capturamos los datos del formulario
        
       terceraPropiedad_.captureModalData();

           // Validar que fundamentoLegal no esté vacío antes de proceder
    if (terceraPropiedad_.anexos.length === 0) {
  ...
  // Agregar los valores de tipoTributario, numeroCuota y anioFiscalg capturados
      ...
      datos.append("resolucionEjecucion", terceraPropiedad_.resolucionEjecucion);
      datos.append("resolucionMedida", terceraPropiedad_.resolucionMedida);
      datos.append("placaVehiculo", terceraPropiedad_.placaVehiculo);
      datos.append("ubicacionPredio", terceraPropiedad_.ubicacionPredio);
      datos.append("partidaRegistral", terceraPropiedad_.partidaRegistral);
       datos.append("fundamento", terceraPropiedad_.fundamento);

        datos.append("anexos", JSON.stringify(terceraPropiedad_.anexos));
        datos.append("numebleInmueble", JSON.stringify(terceraPropiedad_.numebleInmueble));
        ...
        fetch("./vistas/print/imprimirTerceraPropiedad.php", {
            method: "POST",
            body: datos
        })
        .then(response => {
        ...
        .finally(() => {
          ...
          $('#modal_terceria_propiedad').modal('hide');
          ...
          $("#Modalimprimir_cuenta_coactivo").modal("show");
           ...
           terceraPropiedad_.limpiarCampos();
          
      });

      // Limpiar los campos del formulario cuando el modal se cierre
      $('#modal_terceria_propiedad').on('hidden.bs.modal', function () {
          terceraPropiedad_.limpiarCampos();
      });
...
captureModalData() {
    ...
    this.resolucionEjecucion = document.getElementById("resolucionEjecucion19").value;
    this.resolucionMedida = document.getElementById("resolucionMedida19").value;
    this.placaVehiculo = document.getElementById("placaVehiculo19").value;
    this.ubicacionPredio = document.getElementById("ubicacionPredio19").value;
    this.partidaRegistral = document.getElementById("partidaRegistral19").value;
    this.fundamento = document.getElementById("fundamento19").value;
 
    this.anexos=[];
     // Recorrer las filas de la tabla y capturar datos solo si el checkbox está marcado
    $('#modal_terceria_propiedad table tbody tr').each((index, row) => {
        const checkbox = $(row).find('.checkbox'); // Obtener el checkbox
        if (checkbox.is(':checked')) { // Si el checkbox está marcado
       
              const descripcion = $(row).find('td:eq(1) ').text().trim();

            // Asegúrate de que los valores no sean undefined ni null antes de hacer .trim()
            const descripcionVal = descripcion ? descripcion.trim() : ''; // Si resolucion está vacío, usar valor por defecto
...
// Recorrer los checkboxes en lugar de las filas de la tabla
$('.checkboxx').each((index, checkbox) => {
    if ($(checkbox).is(':checked')) { // Si el checkbox está marcado
        const descripcion = $(checkbox).data('descriptionm'); // Obtener el valor de data-description

        // Agregar los datos al arreglo numebleInmueble
        this.numebleInmueble.push({
            descripcion: descripcion
        });

        // Mostrar los resultados en consola para depuración (opcional)
        console.log("Checkbox Capturado:", {
            descripcion: descripcion
        });
    }
});
...
const terceraPropiedad_ = new TerceraPropiedad();
...
$("#anio_orden_propiedad").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaPropiedad tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  terceraPropiedad_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaPropiedad thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  terceraPropiedad_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirPropiedad", function () {
  ...
  terceraPropiedad_.imprimirherecoactivo();
});



//CERRAR VENTANA ES TA VACIO DE COACTIVO CON OK.
$(document).on("click", ".anio_orden_propiedad", function() {
  // Cerrar el modal
  $('#modal_vacio_coactivo').modal('hide');
});

//CLIK PARA MODAL DE ESTADO DE CUENTA COACTIVO
$("#abrirTerceraPropiedad").click(function () {
  $("#modalTerceraPropiedad").modal("show");
});

//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_propiedad", function () {
  //console.log("estas aqui");
  terceraPropiedad_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalTerceraPropiedad').on('show.bs.modal', function () {
   terceraPropiedad_.muestra_deuda();
 });
...
setInterval(terceraPropiedad_.eliminarArchivosPDF, 60000);
