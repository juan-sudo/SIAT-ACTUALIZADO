  ...
  muestra_deuda(){
    ...
    this.idsSeleccionados = [];
        ...
        let self=this;
         ...
         let anoSeleccionado = $('#anio_orden_gestion_domi option:selected').val();
...
actualizarTotales() {
 ...
 $("#segundaTablaGestionDomi tbody td:eq(3)").text(formatearNumero(this.totalImporte));
 $("#segundaTablaGestionDomi tbody td:eq(4)").text(formatearNumero(this.totalGasto));
 $("#segundaTablaGestionDomi tbody td:eq(5)").text(formatearNumero(this.totalSubtotal));
 $("#segundaTablaGestionDomi tbody td:eq(6)").text(formatearNumero(this.totalDescuento));
 $("#segundaTablaGestionDomi tbody td:eq(7)").text(formatearNumero(this.totalTIM));
 $("#segundaTablaGestionDomi tbody td:eq(8)").text(formatearNumero(this.totalTotal));
...
manejarClicSC(thS) {
  const filas = $("#primeraTablaGestionDomi tbody tr");
...
imprimirherecoactivo() {
  ...
  fetch("./vistas/print/imprimirGestionDomicialiaria.php", {
      method: "POST",
      body: datos
  })
  .then(response => {
...
const gestion_domiciliaria_ = new GestionDomicialiaria();
...
$("#anio_orden_gestion_domi").change(function() {
  // Reset all totals
  this.totalGasto = 0;
  this.totalSubtotal = 0;
  this.totalDescuento = 0;
...
$("#primeraTablaGestionDomi tbody").on("click", "tr", function () {
  console.log("Diste click en una fila");
  gestion_domiciliaria_.manejarClicFilaC($(this));
});


// CLIK EN ENCABEZADO DE COACTIVO
$("#primeraTablaGestionDomi thead th:eq(0)").on("click", function () {
  //console.log("acabas de dar click aqui")
  gestion_domiciliaria_.manejarClicSC($(this));
});

//IMPRIMIR ESTADO DE CUENTA COACTIVO

$(document).on("click", "#popimprimirGestionDomiciliaria", function () {
  ...
  gestion_domiciliaria_.imprimirherecoactivo();
});
...
$("#abrirGestionDomiciliaria").click(function () {
  console.log("aqui llego------------");
  $("#modalGestionDomiciliaria").modal("show");
});

//PARA CAMBIO DE SEMESTRE Y AÑO DE FRACCIONAMIENTO
$(document).on("change", "#anio_orden_gestion_domi", function () {
  //console.log("estas aqui");
  gestion_domiciliaria_.muestra_deuda();
 });
  
 // AL ABRIR EL MODAL COACTIVO CARGADO TRIMESTRE
 $('#modalGestionDomiciliaria').on('show.bs.modal', function () {
   gestion_domiciliaria_.muestra_deuda();
 });
...
setInterval(gestion_domiciliaria_.eliminarArchivosPDF, 60000);
